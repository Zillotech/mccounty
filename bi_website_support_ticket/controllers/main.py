...
class EditTicket(http.Controller):
	...
	@http.route(['/edit_ticket/<model("support.ticket"):ticket>'], type='http', auth="public",sitemap=False,website=True)
	...
	@http.route(['/my/save_edit_ticket'], type='http', auth="public", website=True,sitemap=False)
...
class SupportTicket(CustomerPortal):
	...
	@http.route(['/my/ticket', '/my/ticket/page/<int:page>'], type='http', auth="user", website=True,sitemap=False)
	...
	@http.route('/support_ticket', type="http", auth="public", website=True,sitemap=False)
	...
	@http.route('/support_ticket/thanks', type="http", auth="public", website=True,sitemap=False)
	def support_ticket_thanks(self, **post):
		...
		if post:
			...
			_xmlid_lookup('bi_website_support_ticket.group_support_manager')[1]
			group_manager = request.env['res.groups'].sudo().browse(helpdesk_manager_id)
			if group_manager.users:
				for group in group_manager.users:
					template_id = request.env['ir.model.data'].sudo()._xmlid_lookup('bi_website_support_ticket.email_template_support_ticket')[1]
					...
					if template_id:
						...
						values['email_from'] = group.partner_id.email
						if email_from:
							values['email_to'] = email_from
						else:
							values['email_to'] = request.env.user.email
						values['author_id'] = group.partner_id.id
						values['subject'] = name
						mail_mail_obj = request.env['mail.mail'].sudo().create(values)
					   
						if upload_file:
							mail_mail_obj.attachment_ids=[(6,0,[attachment_id.id])]
						if mail_mail_obj:
							mail_mail_obj.send([mail_mail_obj])
						
		return request.render("bi_website_support_ticket.support_thank_you")
	
	@http.route('/ticket/view', type="http", auth="user", website=True,sitemap=False)
	...
	@http.route(['/ticket/view/detail/<model("support.ticket"):ticket>'],type='http',auth="public",website=True,sitemap=False)
	...
	@http.route(['/ticket/message'],type='http',auth="public",website=True,sitemap=False)
	...
	@http.route(['/ticket/comment/<model("support.ticket"):ticket>'],type='http',auth="public",website=True,sitemap=False)
	...
	@http.route(['/support_ticket/comment/send'],type='http',auth="public",website=True,sitemap=False)
