...
class support_ticket(models.Model):
	...
	_inherit = ['portal.mixin',  'utm.mixin', 'rating.mixin', 'mail.activity.mixin']
	...
	def create(self, vals):
		for val in vals:
			if val.get('sequence', _('Ticket')) == _('Ticket'):
				val['sequence'] = self.env['ir.sequence'].next_by_code('support.ticket') or _('Ticket')
			if str(val.get('date_close')) < str(val.get('date_create')):
	...
	stage_id = fields.Many2one ('support.stage', 'Stage', index=True,tracking=True)
...
class ResPartner(models.Model):
	...
	def signup_get_auth_param(self):
		...
		"""
		ticket = self.env['support.ticket'].sudo().search([('partner_id','=',self.id)],limit=1)
		if ticket:
			pass
		else:
