...
class task_wizard(models.Model):
	...
	def default_get(self, flds): 
		...
		suport_ticket_id = self.env['support.ticket'].sudo().browse(self._context.get('active_id'))
		if 'name' in flds:
			result['name'] = suport_ticket_id.name
		if 'description' in flds:
			result['description'] = suport_ticket_id.description
		if 'deadline_date' in flds:
			result['deadline_date'] = datetime.now()
		if 'project_id' in flds:
	...
	def create_task(self):
		support_ticket_id = self.env['support.ticket'].sudo().browse(self._context.get('active_id'))
